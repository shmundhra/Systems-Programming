...
#define debugI( ds_name ) fprintf(stderr , "%15s:: " , #ds_name) ; for ( auto val : ds_name ) fprintf(stderr,"%8.0lf ",val); cerr<<"\n";
#define debug( ds_name )  fprintf(stderr , "%15s:: " , #ds_name) ; for ( auto val : ds_name ) fprintf(stderr,"%8.3lf ",val); cerr<<"\n";
#define debug1(x)  fprintf(stderr , "%15s:: " , #x ) ; cerr<<x<<"\n" ;
#define DELTA 2 
...
{   
    ...
    for( int i = 0 ; !(Job_PQueue.empty() and i == N)  ; )
    {
        ...
        Curr_Job = Job_PQueue.top() ; Job_PQueue.pop() ; 
...
{
    double Curr_Time, Curr_EndTime , Curr_Job_Index , Next_StartTime ;  
    double Turnaround , Avg_Turnaround  ; 
    SJF_Info Curr_Job ; 
...
{   
    int Not_Processed , count ;  
    int Last_Job_Index , Curr_Job_Index ;   
    ...
    Last_Job_Index = -1 ; 
    Curr_Time = 0 ; 
    /* Last_Job_Index is the index of the last job processed, initially -1 */
    while(1)
    {           
        count = 0 ; 
        /*Pick up the First Job whose arrival time <= Curr_time */
        for ( int i = Last_Job_Index + 1 ; count <= N  ; i++ , count++ )
        {               
            if ( Job_Queue[i%N].Arrival <= Curr_Time and !Processed[i%N] )
            {   
                Curr_Job = &Job_Queue[i%N] ; 
                Curr_Job_Index = i%N ; 
                break ; 
            }
        }
        if ( count > N )
        {
            Curr_Time = Job_Queue[(Last_Job_Index+1)%N].Arrival ; 
            continue ;
        }
        //debug1(Curr_Job_Index) ; 
        Schedule[Curr_Job_Index].push_back(Curr_Time) ; 

        /*Curr_Job is longer than time quantum*/
        if ( DELTA < Curr_Job->Burst )
        {   
            Curr_EndTime = Curr_Time + DELTA ;          
            Curr_Job->Burst -= DELTA ;          
        }
        else /* Curr_Job runs to completion */
        {
            Curr_EndTime = Curr_Time + Curr_Job->Burst ; 
            Turnaround += Curr_EndTime - Curr_Job->Arrival ; 
            
            Processed[Curr_Job_Index] = true ;      
            if ( (--Not_Processed)  == 0 ) {
                break ; 
            }
        }   
        Curr_Time = Curr_EndTime ; 
        Last_Job_Index = Curr_Job_Index  ;  
    }
    //for ( int i = 0 ; i < N ; i++ ){ fprintf(stderr , "%12s[%d]:: " , "Schedule" , i ) ; for ( auto val : Schedule[i] ) fprintf(stderr,"%6.3lf ",val); cerr<<"\n"; } 
...
{   
    ...
    { 
        ...
        Curr_Job_Index = -(Job_PQueue.begin()->second) ; debug1(Curr_Job_Index) ; debug(RespRatio) ;  
...
int main( int argc , char* argv[] ) 
{
    ...
    for( int i = 0 ; i < N ; i++ ) Process_Id.push_back(i) ; 
